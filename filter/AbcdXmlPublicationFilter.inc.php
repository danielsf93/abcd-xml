...
/**
 * @file plugins/importexport/abcd/filter/AbcdXmlPublicationFilter.inc.php
 *
 * Copyright (c) 2014-2021 Simon Fraser University
 * Copyright (c) 2000-2021 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * @class AbcdXmlPublicationFilter
...
class AbcdXmlPublicationFilter extends AbcdXmlPKPPublicationFilter {
	...
	function getClassName() {
		return 'plugins.importexport.abcd.filter.AbcdXmlPublicationFilter';
	}
	...
	function populateObject($publication, $node) {
		...
		if ($seriesPath !== '') {
			...
			if (!$series) {
				$deployment->addError(ASSOC_TYPE_PUBLICATION, $publication->getId(), __('plugins.importexport.abcd.error.unknownSeries', array('param' => $seriesPath)));
			} else {
	...
	function getImportFilter($elementName) {
		...
		$importFilters = $filterDao->getObjectsByGroup('abcd-xml=>' . $importClass);
