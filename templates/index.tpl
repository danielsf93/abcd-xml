{**
 * plugins/importexport/abcd/templates/index.tpl
 *
...
<div id="importExportTabs" class="pkp_controllers_tab">
	<ul>
		<li><a href="#import-tab">{translate key="plugins.importexport.abcd.import"}</a></li>
		<li><a href="#export-tab">{translate key="plugins.importexport.abcd.export"}</a></li>
	</ul>
	<div id="import-tab">
		<script type="text/javascript">
			$(function() {ldelim}
		...
		<form id="importXmlForm" class="pkp_form" action="{plugin_url path="importBounce"}" method="post">
			...
			{fbvFormArea id="importForm"}
				...
				{fbvFormArea id="file"}
					{fbvFormSection title="plugins.importexport.abcd.import.instructions"}
						{include file="controllers/fileUploadContainer.tpl" id="plupload"}
					{/fbvFormSection}
				{/fbvFormArea}

				{fbvFormButtons submitText="plugins.importexport.abcd.import" hideCancel="true"}
			{/fbvFormArea}
		</form>
	</div>
	<div id="export-tab">
		{if !$currentContext->getData('publisher') || !$currentContext->getData('location') || !$currentContext->getData('codeType') || !$currentContext->getData('codeValue')}
			...
			{translate key="plugins.importexport.abcd.onix30.pressMissingFields" url=$contextSettingsUrl}
		{/if}
		<script type="text/javascript">
			$(function() {ldelim}
				// Attach the form handler.
			...
			{fbvFormArea id="exportForm"}
				...
				{fbvFormSection}
					...
					<pkp-button @click="submit('#exportXmlForm')">
						{translate key="plugins.importexport.abcd.exportSubmissions"}
					</pkp-button>
				{/fbvFormSection}
			{/fbvFormArea}
		</form>
