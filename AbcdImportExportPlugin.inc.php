...
/**
 * @file plugins/importexport/abcd/AbcdImportExportPlugin.inc.php
 *
 * Copyright (c) 2014-2021 Simon Fraser University
 * Copyright (c) 2003-2021 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * @class AbcdImportExportPlugin
 * @ingroup plugins_importexport_abcd
 *
 * @brief Abcd XML import/export plugin
 */

import('lib.pkp.classes.plugins.ImportExportPlugin');

class AbcdImportExportPlugin extends ImportExportPlugin {
	/**
	 * Constructor
	 */
	...
	function register($category, $path, $mainContextId = null) {
		...
		if ($success && $this->getEnabled()) {
			$this->addLocaleData();
			$this->import('AbcdImportExportDeployment');
		}
		return $success;
	}
	...
	function getDisplayName() {
		return __('plugins.importexport.abcd.displayName');
	}
	...
	function getDescription() {
		return __('plugins.importexport.abcd.description');
	}
		...
		switch (array_shift($args)) {
			...
			case 'importBounce':
				...
				$json->setEvent('addTab', array(
					'title' => __('plugins.importexport.abcd.results'),
			...
			case 'import':
				...
				if (!$temporaryFile) {
					$json = new JSONMessage(true, __('plugins.inportexport.abcd.uploadFile'));
					header('Content-Type: application/json');
					return $json->getString();
				}
				$temporaryFilePath = $temporaryFile->getFilePath();

				$deployment = new AbcdImportExportDeployment($context, $user);
	...
	function exportSubmissions($submissionIds, $context, $user, $opts = array()) {
		...
		$abcdExportFilters = $filterDao->getObjectsByGroup('monograph=>abcd-xml');
		assert(count($abcdExportFilters) == 1); // Assert only a single serialization filter
		$exportFilter = array_shift($abcdExportFilters);
		$exportFilter->setDeployment(new AbcdImportExportDeployment($context, $user));
		$submissions = array();
	...
	function importSubmissions($importXml, $deployment) {
		$filterDao = DAORegistry::getDAO('FilterDAO'); /* @var $filterDao FilterDAO */
		$abcdImportFilters = $filterDao->getObjectsByGroup('abcd-xml=>monograph');
		assert(count($abcdImportFilters) == 1); // Assert only a single unserialization filter
		$importFilter = array_shift($abcdImportFilters);
		...
		switch ($command) {
			case 'import':
				...
				if (!$user) {
					if ($userName != '') {
						echo __('plugins.importexport.common.cliError') . "\n";
						echo __('plugins.importexport.abcd.error.unknownUser', array('userName' => $userName)) . "\n\n";
					}
					$this->usage($scriptName);
					return;
				}
				...
				$deployment = new AbcdImportExportDeployment($press, $user);
