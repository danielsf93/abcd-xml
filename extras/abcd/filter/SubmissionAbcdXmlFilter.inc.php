...
/**
 * @file plugins/importexport/abcd/filter/SubmissionAbcdXmlFilter.inc.php
 *
 * Copyright (c) 2014-2021 Simon Fraser University
 * Copyright (c) 2000-2021 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * @class SubmissionAbcdXmlFilter
 * @ingroup plugins_importexport_abcd
 *
 * @brief Base class that converts a set of submissions to a Abcd XML document
 */

import('plugins.importexport.abcd.extras.abcd.filter.AbcdExportFilter');

class SubmissionAbcdXmlFilter extends AbcdExportFilter {
	...
	function __construct($filterGroup) {
		$this->setDisplayName('Abcd XML submission export');
	...
	function getClassName() {
		return 'plugins.importexport.abcd.extras.abcd.filter.SubmissionAbcdXmlFilter';
	}
	...
	function addFiles($doc, $submissionNode, $submission) {
		...
		foreach ($submissionFilesIterator as $submissionFile) {
			...
			if (in_array($submissionFile->getData('fileStage'), [SUBMISSION_FILE_QUERY, SUBMISSION_FILE_NOTE, SUBMISSION_FILE_REVIEW_ATTACHMENT])) {
				$this->getDeployment()->addWarning(ASSOC_TYPE_SUBMISSION, $submission->getId(), __('plugins.importexport.abcd.error.submissionFileSkipped', array('id' => $submissionFile->getId())));
				continue;
			}

			$abcdExportFilters = $filterDao->getObjectsByGroup(get_class($submissionFile) . '=>abcd-xml');
	...
	function addPublications($doc, $submissionNode, $submission) {
		$filterDao = DAORegistry::getDAO('FilterDAO'); /** @var $filterDao FilterDAO */
		$abcdExportFilters = $filterDao->getObjectsByGroup('publication=>abcd-xml');
		assert(count($abcdExportFilters)==1); // Assert only a single serialization filter
		$exportFilter = array_shift($abcdExportFilters);
		$exportFilter->setDeployment($this->getDeployment());
