...
/**
 * @file plugins/importexport/abcd/filter/PKPPublicationAbcdXmlFilter.inc.php
 *
 * Copyright (c) 2014-2021 Simon Fraser University
 * Copyright (c) 2000-2021 John Willinsky
 * Distributed under the GNU GPL v3. For full terms see the file docs/COPYING.
 *
 * @class PKPPublicationAbcdXmlFilter
...
class PKPPublicationAbcdXmlFilter extends AbcdExportFilter {
	...
	function __construct($filterGroup) {
		$this->setDisplayName('Abcd XML Publication export');
	...
	function addAuthors($doc, $entityNode, $entity) {
		...
		$abcdExportFilters = $filterDao->getObjectsByGroup('author=>abcd-xml');
	...
	function addRepresentations($doc, $entityNode, $entity) {
		$filterDao = DAORegistry::getDAO('FilterDAO'); /** @var $filterDao FilterDAO */
		$abcdExportFilters = $filterDao->getObjectsByGroup($this->getRepresentationExportFilterGroupName());
		assert(count($abcdExportFilters)==1); // Assert only a single serialization filter
		$exportFilter = array_shift($abcdExportFilters);
		$exportFilter->setDeployment($this->getDeployment());
